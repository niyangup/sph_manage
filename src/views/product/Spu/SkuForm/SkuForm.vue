...
export default {
  ...
  methods: {
    ...
    async getData(category1Id, category2Id, spu) {
      ...
      this.spu = spu
      // 获取图片的数据
      const result = await Api.spu.reqSpuImageLIst(spu.id)
      if (result.code === 200) {
      ...
      // 获取销售属性的数据
      const result1 = await Api.spu.reqSpuSaleAttrList(spu.id)
      if (result1.code === 200) {
        this.spuSaleAttrList = result1.data
      }
      // 获取平台属性的数据
      const result2 = await Api.spu.reqAttrInfoList(
        category1Id,
        category2Id,
        spu.category3Id
      )
    ...
    async save() {
      ...
      // 发请求
      const result = await Api.spu.reqAddSku(skuInfo)
      if (result.code === 200) {
