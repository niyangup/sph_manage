...
export default {
  ...
  methods: {
    ...
    async getSpuList(pages = 1) {
      ...
      const result = await Api.spu.reqSpuList(page, limit, category3Id)
      if (result.code === 200) {
    ...
    // 删除SPU的回调
    async deleteSpu(row) {
      const result = await Api.spu.reqDeleteSpu(row.id)
      if (result.code === 200) {
    ...
    async handler(spu) {
      ...
      this.spu = spu
      // 获取sku列表的数据进行展示
      const result = await Api.spu.reqSkuList(spu.id)
      if (result.code === 200) {
